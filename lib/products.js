...
var complex = exports.complex = {
  /**
   * ComplexList of products. Used for GetMyFeesEstimate request
   */
  CreateFeesEstimateList: function () {
    var obj = new mws.ComplexList('FeesEstimateRequestList.FeesEstimateRequest');
    obj.add = function (valuesObject) {
      var memberValues = {
        MarketplaceId: valuesObject.marketplaceId,
        IdType: valuesObject.idType,
        IdValue: valuesObject.id,
        IsAmazonFulfilled: valuesObject.fba,
        Identifier: valuesObject.identifier,
        'PriceToEstimateFees.ListingPrice.CurrencyCode': valuesObject.currency,
        'PriceToEstimateFees.ListingPrice.Amount': valuesObject.price
      };
      
      if (valuesObject.shipping) {
        memberValues['PriceToEstimateFees.Shipping.CurrencyCode'] = valuesObject.currency;
        memberValues['PriceToEstimateFees.Shipping.Amount'] = valuesObject.shipping;
      }
      
      if (valuesObject.points) {
        memberValues['PriceToEstimateFees.Points.PointsNumber'] = valuesObject.points;
      }
      
      obj.members.push(memberValues);
      return obj;
    };
    return obj;
  }
...
var calls = exports.requests = {
  ...
  GetMyFeesEstimate: function () {
    return new ProductsRequest('GetMyFeesEstimate', {
      //MarketplaceId: { name: 'MarketplaceId', required: true },
      FeesEstimateRequestList: { name: 'FeesEstimateRequestList', type: 'Complex', required: true, construct: complex.CreateFeesEstimateList }
    });
  },
};
